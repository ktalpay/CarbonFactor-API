# Roadmap (Conservative)

## Near-term

1. Strengthen contract and catalog tests with additional negative-path coverage.
2. Introduce a minimal HTTP server adapter that reuses the existing transport boundary once contract stability improves.
3. Define configuration profile for swapping synthetic data with validated local data artifacts.

## Integration Direction

- CarbonFactor Parser integration is planned as a future data source path only; it is not implemented in this repository.
- CarbonOps Assistant integration is planned as a future consumer path only; it is not implemented in this repository.

## Non-goals for this phase

- Production deployment
- External API integrations
- Database infrastructure
- Publish/release automation

- Added local-only FastAPI adapter as pre-alpha transport wiring; production hardening remains out of scope.
